...
public class AiImageService {
	...
	public AiImageResponseDto generateImage(String koreanKeyword, String category, String description) {
		validateRequiredInputs(koreanKeyword, category);

		log.info("AI 이미지 생성 요청 시작 - 키워드: {}, 업종: {}, 추가 설명: {}", koreanKeyword, category, description);

		AiImageRequestDto requestDto = new AiImageRequestDto(koreanKeyword, category, description);

		try {
			return restClient.post().uri(GENERATE_ENDPOINT).contentType(MediaType.APPLICATION_JSON).body(requestDto)
