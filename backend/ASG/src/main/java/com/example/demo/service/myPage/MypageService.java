...
public class MypageService {
	...
	public ContentSettingsResponse getContentSettings(Long userId) {
	    return contentSettingsRepository.findByUser_Id(userId)
 ...
 // ── 영업시간 조회 ──────────────────────────────────────────
    ...
    public List<Map<String, Object>> getBusinessHours(Long userId) {
        return businessHoursRepository.findByUser_IdOrderByDayOfWeekAsc(userId)
                .stream()
                .map(bh -> {
 ...
 // ── 콘텐츠 히스토리 조회 ────────────────────────────────
    ...
    public List<ContentHistoryResponse> getContentHistory(Long userId) {
        Brand brand = brandRepository.findByUser_Id(userId)
                .orElseThrow(() -> new IllegalArgumentException("브랜드를 찾을 수 없습니다."));
        return contentPostRepository.findByBrandIdOrderByPublishedAtDesc(brand.getBrandId())
            .stream()
