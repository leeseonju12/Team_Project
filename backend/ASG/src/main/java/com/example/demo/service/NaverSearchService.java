...
public class NaverSearchService {
    ...
    private List<String> getCandidateKeywords(String industryType) {
        return switch (industryType) {
            case "카페 / 베이커리"   -> List.of("카페 추천", "베이커리 맛집", "브런치 카페", "디저트 카페", "신메뉴");
            case "음식점 / 식당"     -> List.of("맛집 추천", "점심 특선", "혼밥 맛집", "배달 맛집", "저녁 맛집");
            case "미용 / 뷰티"       -> List.of("미용실 추천", "헤어샵 예약", "네일샵 추천", "뷰티샵 추천", "염색 잘하는 곳");
            case "패션 / 의류"       -> List.of("옷 쇼핑", "빈티지샵 추천", "코디 추천", "의류 신상", "패션 추천");
            case "숙박 / 펜션"       -> List.of("펜션 추천", "글램핑 추천", "게스트하우스 추천", "숙박 예약", "감성 숙소");
            case "피트니스 / 스포츠" -> List.of("헬스장 추천", "PT 가격", "필라테스 추천", "요가 학원", "퍼스널트레이닝");
            case "교육 / 학원"       -> List.of("학원 추천", "과외 추천", "성인 영어학원", "자격증 학원", "공방 추천");
            case "의료 / 병원"       -> List.of("한의원 추천", "치과 추천", "피부과 추천", "동네병원 추천", "한방치료");
            case "소매 / 쇼핑"       -> List.of("편집샵 추천", "소품샵 추천", "신상 입고", "독립서점 추천", "잡화 쇼핑");
            case "기타"              -> List.of("생활편의 추천", "세탁소 추천", "반찬가게 추천", "동네가게 추천", "소상공인 추천");
