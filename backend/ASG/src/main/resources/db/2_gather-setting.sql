/*
  ...
  STEP 6.  아래 SET 두 줄을 실제 id 값으로 수정 후 이 파일 전체 실행

           SET @DIAZ_users.id     = 1;  ← 카페 디아즈로 쓸 users.id
           SET @SANJEONG_USER_ID = 2;  ← 카페 산정으로 쓸 users.id
...
-- ──────────────────────────────────────────────────────────────────
SET @DIAZ_USER_ID     = 1;  -- ← 카페 디아즈로 쓸 users.id
SET @SANJEONG_USER_ID = 2;  -- ← 카페 산정으로 쓸 users.id
