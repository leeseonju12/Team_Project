...
function buildGenerateRequestParams() {
  ...
  return {
    ...
    payload: {
      ...
      maxLength: rangeEl ? parseInt(rangeEl.value, 10) : 300
    }
  };
}
...
window.generateContent = async function () {
  ...
  if (!validateGenerateRequest(requestMeta)) {
    return;
  }
...
window.publishPost = async function (sns) {
  ...
  try {
    ...
    if (result.status === 'success') {
      ...
      uiManager.showToast(`⚠️ 게시 실패: ${result.message || '알 수 없는 오류'}`);
    }
  } catch (error) {
...
function addToPending(sns, menu, bodyText) {
  const platform = PLATFORM_CONFIG[sns] || { label: sns, color: '#6366F1' };
    ...
    .map(
      (post) => `
        <div class="pending-item" id="${post.id}" draggable="true"
          ...
          <div class="pi-info">
            <div class="pi-name">${post.menu}</div>
